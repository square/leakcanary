...
public final class HeapAnalyzer {
    ...
    for (int i = 1; i < lastElementIndex; i++) {
      ...
      if (reachability.status == Reachability.Status.UNKNOWN) {
        if (i <= lastReachableElementIndex) {
          // TODO Add @reference to name.
          String lastReachableName = elements.get(lastReachableElementIndex).getSimpleClassName();
          expectedReachability.set(i, Reachability.reachable(lastReachableName + " is alive"));
          ...
          expectedReachability.set(i,
              Reachability.unreachable(firstUnreachableName + " is not alive"));
        }
      }
    }
  ...
  private LeakTraceElement buildLeakElement(LeakNode node) {
    ...
    String uniqueId =
        (holder instanceof ClassObj) ? "" : String.format(Locale.US, "@%d", holder.getUniqueId());
    return new LeakTraceElement(uniqueId, node.leakReference, holderType, classHierarchy, extra,
