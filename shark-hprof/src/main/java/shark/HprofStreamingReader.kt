...
class HprofStreamingReader internal constructor(
  private val hprof: HprofFile
) {

  /**
   * Reads all hprof records from [reader] and calls [listener] back for each record that matches
   * one of the provided [recordTypes].
   */
  @Suppress("ComplexMethod", "LongMethod", "NestedBlockDepth")
  fun readHprofRecordsAsStream(
    ...
    Okio.buffer(Okio.source(hprof.file.inputStream())).use { source ->
      val reader = HprofRecordReader(hprof, source)
      val startPosition = hprof.recordsPosition
