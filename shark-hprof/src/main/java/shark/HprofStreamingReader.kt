...
class HprofStreamingReader private constructor(
  private val openRecordReader: () -> ReaderSource
) {

  private data class ReaderSource(
    val reader: HprofRecordReader,
  ...
  fun readHprofRecords(
    ...
    val (reader, exhausted) = openRecordReader()
  ...
  companion object {
    ...
    /**
     * Creates a [HprofStreamingReader] for the provided [hprofFile]. [header] will be read from
     * [hprofFile] unless you provide it.
     */
    fun createStreamingReaderFor(
      hprofFile: File,
      header: HprofHeader = HprofHeader.parseHeaderOf(hprofFile)
    ): HprofStreamingReader {
      return createStreamingReaderFor(header) {
        Okio.source(hprofFile.inputStream())
      }
    }

    /**
     * Creates a [HprofStreamingReader] that will call [sourceProvider] on every
     * [readHprofRecords] to obtain a [Source] to read the hprof data from. Before reading the
     * hprof records, [HprofStreamingReader] will skip [HprofHeader.recordsPosition] bytes.
     */
    fun createStreamingReaderFor(
      header: HprofHeader,
      sourceProvider: () -> Source
    ): HprofStreamingReader {
      return HprofStreamingReader {
        val source = Okio.buffer(sourceProvider())
