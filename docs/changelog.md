...
* 🐛 [#2700](https://github.com/square/leakcanary/issues/2700) `ObjectDominators.buildDominatorTree()` and `ObjectDominators.renderDominatorTree()` counted the content of an object once per object holding it, so they reported far more retained than the heap holds — on that issue, a total retained size that wrapped past `Int.MAX_VALUE` and came out negative. The traversal skips the reference from a `java.lang.String` to its value array, and the references from a `java.lang.Integer[]` and its equivalents for the other primitive types to the boxed primitives they hold, because a leak trace never needs to name them, and the shallow size of the object holding the content is inflated by the size of that content to make up for it. That's exact while each piece of content has a single holder, and content is shared more often than it looks: `new String(String)` copies the reference to the value array rather than the array, `Integer.valueOf()` caches -128 to 127, `Boolean.valueOf()` returns one of two instances, and before Android Marshmallow `String.substring()` shared its parent's array. The dominator tree now follows those references, so shared content is attributed to a single dominator and counted once. On a JVM heap dump of 6669985 bytes holding 200 strings that share one 2 MB array, the total retained size goes from 205816571 bytes down to 6669985, i.e. the size of the heap.
    * ⚠️ The new `ContentReferences` enum says whether a traversal follows content references, and `ActualMatchingReferenceReaderFactory`, `FieldInstanceReferenceReader`, `ObjectArrayReferenceReader` and `AndroidObjectSizeCalculator` take one. It defaults to `ContentReferences.SKIPPED`, which is what they all did before, so calling code doesn't change — but the constructors have a new parameter, so code compiled against a previous alpha has to be recompiled. A reference reader and the `ObjectSizeCalculator` used on the same traversal have to be given the same value, or content is either counted twice or not at all.
    * 🔀 The leak analysis and heap growth detection still skip content references, so the retained sizes they report still count content once per object holding it. Following the references means reading a heap dump record per string reached, which is worth it for a dominator tree of the whole heap and isn't for an analysis running on a phone: on the Android heap dumps in our test resources, following them takes the analysis from 11786, 17407 and 19711 random access reads to 22592, 64520 and 32494, and from 511, 696 and 447 distinct heap dump pages read to 543, 1752 and 994.
