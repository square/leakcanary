...
* 🔨 `HeapDominatorTree`, and `ObjectDominators.buildDominatorTree` with it, needs about half the memory it did. Four changes: its two vertex maps are keyed by an object's index in the heap dump rather than by its id, so they're `IntArray`s sized once instead of a `MutableLongIntMap` that grows and rehashes; the reference that discovers a vertex is no longer stored a second time as one of that vertex's predecessors, which is more than half the references in a heap dump; a vertex's predecessors are one contiguous slice rather than a linked list, 4 bytes an edge rather than 8; and the immediate dominators are computed with SEMI-NCA rather than Lengauer-Tarjan, which needs three arrays per vertex where Lengauer-Tarjan needs six. On a generated 487 MB heap dump of 9.24 million reachable objects and 17.35 million references between them, the smallest heap the build completes in goes from 1025 MB to 525 MB, and the build itself from 19.4 s to 15.4 s. Neither class is used by the heap analysis, on device or off it — they're for tools that want the exact dominator tree of a whole heap dump, which is what Shark Explorer is built on.
    * 🔀 Mapping an object id to its index is a binary search where a hash lookup used to do. It comes out free here, because the id has to be resolved to an index anyway: the traversal then reads each object positionally, which drops the binary search `findObjectById` used to do per vertex.
    * 🔀 A heap dump holding a reference to an object id that isn't in it now fails with an `IllegalArgumentException` naming the heap dump as corrupt, where it used to fail one level down with `findObjectById`'s *"Object id … not found in heap dump"*.
