...
* 🔨 The set of objects a traversal has already visited is now stored as one bit per object rather than as a hash set of object ids. Hashing 8 byte addresses into a power of two array cost `nextPowerOfTwo(ceil(count / 0.75)) * 8` bytes, 86 to 143 times more than one bit per object on the heap dumps we measured: on our benchmark heap dump that set goes from 2.10 MB to 16.2 kB, and on a 120 million object JVM heap dump from 2.15 GB to 15.0 MB, where it also brings the memory the whole analysis needs down from 16 GB to 13 GB. The set is now sized exactly up front instead of doubling as the traversal fills it, and is at most 268 MB for any heap dump. In exchange the traversal looks up the object index of every reference it follows, which measured 10% slower on that 120 million object heap dump.
    * 💥 Analyses of heap dumps with more than 805306368 reachable objects failed with *"Maximum array size exceeded for this load factor"*, however much memory the analysis was given. Shark's hash structures address their slots with an `Int`, so their hash array can't grow past 2^30 slots, which at a load factor of 0.75 caps them at 805306368 elements. The visited set was the structure that reached that, and it isn't keyed on object ids anymore. That message now says how many elements the structure can hold, that the limit is Shark's rather than the heap dump format's, and what to use instead, since the sets that track the traversal frontier are still keyed on object ids.
    * 💥 Heap growth detection had the same problem with a different ceiling and a worse error: its visited set was an `androidx.collection.MutableLongSet`, which holds at most 939524096 elements and fails with `NegativeArraySizeException: -268435455` past that.
    * 🔨 The visited set used to be sized up front at half the number of instances in the heap dump, which on a heap dump with 918 million instances allocates 8.59 GB before visiting anything (4.90 GB in heap growth detection, which used a different set implementation). One bit per object has nothing to estimate, so there's no estimate left to get wrong.
