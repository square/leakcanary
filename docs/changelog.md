...
* 🐛 Heap growth detection computed the retained size of the objects it reports as growing from an approximate dominator tree built as the heap was traversed, an approximation that over attributes, sometimes by a lot. Retained sizes are now computed with the LeakShare metric from [BLeak](https://plasma-umass.org/bleak-paper.pdf), the paper heap growth detection is based on: the objects that are only reachable through the reported growing objects are credited to the growing objects that reach them.
