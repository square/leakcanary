...
* 🔀 A leak whose GC root is one of the seven root types LeakCanary only started reporting in this release — `Unknown GC root type`, `Interned string`, `Object awaiting finalization`, `Debugger reference`, `Reference cleanup`, `VM internal reference` and `Unreachable object` — is dropped by Android Studio's Profiler, along with every other leak in the same analysis. Studio parses the analysis out of logcat and rejects a `GC Root:` description it doesn't know, and it only knows the nine types that existed before. LeakCanary 2 never emitted these because building the leak trace threw on them instead, so the reports Studio was written against could not contain them. Fixing this needs a Studio release; the leaks are still reported in full on the device and by `shark-cli`.
