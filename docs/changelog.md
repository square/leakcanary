...
* 🐛 [#2790](https://github.com/square/leakcanary/issues/2790) [#1670](https://github.com/square/leakcanary/issues/1670) LeakCanary deleted heap dumps whose analysis hadn't run yet. It keeps at most `LeakCanary.Config.maxStoredHeapDumps` heap dumps, 7 by default, and enforced that by deleting the oldest ones every time it created a new heap dump file, without checking whether an analysis was still queued for any of them. An analysis runs on WorkManager, which survives process death and reboots, so it can start minutes or days after the heap dump was taken and would then find nothing to read and fail with *"Hprof file … missing"*. The cleanup now deletes the heap dumps a stored analysis was already run on first, and only takes one that's still waiting when there's no analyzed heap dump left to delete — the limit is still a limit, so a device dumping the heap faster than it can analyze it doesn't fill up the disk.
