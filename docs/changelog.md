...
* 💥 [#2773](https://github.com/square/leakcanary/issues/2773) The path finding traversal kept a `ReferencePathNode.ChildNode` per reference it had followed and not released yet — the frontier of the breadth first traversal, plus the ancestors those nodes keep alive so that a path can be reported — and each of those nodes held the lazy resolver of the reference that led to it, a closure capturing what's needed to name that reference in a leak trace. Sampled 3 seconds into the analysis of a 4.4 million object Android heap dump, that's 63 MB in 2649004 nodes and 33 MB in the 2649002 closures hanging off them, and only the handful of nodes on a path that ends up in a leak trace ever need those details. A queued node is now an internal type holding its parent, the index of its object in the heap dump, and the index of the reference that led to it within the sequence its parent's `ReferenceReader` returned: 37 MB for 2367147 nodes at that same sample, with nothing hanging off them. The details of a reported path are recovered by reading its parent's references again and picking that one out, which relies on a `ReferenceReader` returning the same references in the same order every time it reads the same object, and is checked when they're resolved. On that heap dump the smallest heap the analysis completes in goes from 313 MB to 193 MB. (The 321 MB reported above for the same starting point was bisected on a coarser ladder.) On a heap dump of the same object count whose objects hang off 1000 deep chains, where what the queue costs is the ancestors each queued node keeps alive rather than the width of the frontier, it goes from 305 MB to 153 MB.
    * 🔨 Identifying a queued object by its index rather than by its id is also what lets the traversal dequeue with the positional `HeapGraph.findObjectByIndex` instead of `findObjectById`, whose binary search repeats the one the visited set already did when the node was enqueued. Path finding is 6.6% to 10.2% faster on the Android heap dumps in our test resources, which more than pays back the 4% to 8% that keying the visited set by object index cost above.
* 💥 [#2783](https://github.com/square/leakcanary/issues/2783) Heap growth detection ran out of memory on heaps that a single analysis copes with, because the set of objects its traversal had already visited was keyed by object id and sized from a guess: `instanceCount / 2`, allocated eagerly, before reading a single reference. That's 4.83 GB of empty hash table on a heap dump of 918610159 instances, and the guess is always too small anyway, since on the Android heap dumps in our test resources the traversal visits 0.65x to 1.08x of `instanceCount`, so the table then doubled and rehashed into a new one while still holding the old. It's now one bit per object in the heap dump, keyed by the object's index rather than by its id, allocated once at a size that's known upfront, so it can't grow, rehash, or be sized from a guess: 508 KB rather than 37.7 MB on a heap of 4.06 million objects. The smallest heap growth detection completes a two heap dump comparison of that size in goes from 232 MB to 208 MB.
    * 🔀 Mapping an object id to its index is a binary search where a hash lookup used to do, and the traversal does that once per reference it reads, once per heap dump in the sequence. A single traversal of the Android heap dumps in our test resources is 5.4% to 11.8% slower, and the two heap dump comparison above is 4% to 5% slower.
...
* 🐛 [#2777](https://github.com/square/leakcanary/issues/2777) An object whose record takes more than 2 GB in the heap dump reported a negative size. `HeapObject.recordSize` and the `byteSize` of an array were `Int`, narrowed from the `Long` the index holds, so a 2147483672 byte array record came back as -2147483624 and every size derived from it was wrong, including the shallow and retained sizes an analysis reports. Sizes are now `Long` from the index all the way out to `ObjectSizeCalculator`.
    * ⚠️ `HeapObject.recordSize`, `HeapInstance.byteSize`, `HeapObjectArray.byteSize`, `HeapPrimitiveArray.byteSize`, `ObjectSizeCalculator.computeSize()` and `ObjectDominators.DominatorNode.shallowSize` are `Long` instead of `Int`. `HeapClass.instanceByteSize` stays an `Int`: it's the instance size field of the class dump record, which the format bounds to what a `u2` field count can describe.
* ⚠️ `HeapPrimitiveArray.readByteSize()` is gone. It has been deprecated in favour of the `byteSize` property since 2.10, and the identical method on `HeapObjectArray` was removed in 3.0 Alpha 1, which missed this one.
* 🐛 A random access read of more than 2 GB from a heap dump reported that it had read 0 bytes without reading any, because `FileSourceProvider` narrowed the byte count to an `Int`. Reading a single record that large is what the item above makes possible.
* 🔀 Creating a `Reference` with `valueObjectId` set to a null reference now fails with an `IllegalArgumentException` that says how to fix it, instead of the analysis failing later on (or silently dropping the reference). `ReferenceReader` implementations must skip null references, e.g. by reading ids with `HeapValue.asNonNullObjectId` rather than `HeapValue.asObjectId`, which returns 0 for a null reference.
* 🐛 [#2789](https://github.com/square/leakcanary/issues/2789) When the analysis ran out of memory, LeakCanary was meant to replace the failure with guidance on what to do about it — kill the app, set `largeHeap=true`, run the analysis in a separate process, or analyze the heap dump with `shark-cli`. It only ever did so when `OutOfMemoryError` was the direct cause of the failure, which it almost never is: Shark's hash maps catch it while allocating their buffers and rethrow it wrapped in a `RuntimeException` naming the buffer they failed to allocate, and that's how the analysis usually runs out of memory. The whole cause chain is now searched, so those failures report the guidance instead of a stack trace inviting a bug report.
* 🐛 [#2773](https://github.com/square/leakcanary/issues/2773) The heap analysis failure screen asked for a bug report even when the analysis had run out of memory, which is how most of the out of memory reports on the issue tracker were filed. It now points at running the analysis again after killing the app, or sharing the heap dump to analyze it on a computer, and the *"file a bug report"* text and the link that opens a new GitHub issue with the failure in the clipboard are only shown for failures that actually are worth reporting.
* 🔨 [#2783](https://github.com/square/leakcanary/issues/2783) Heap growth detection that ran out of memory reported a stack trace, where heap analysis reports guidance on what to do about it. It now reports guidance too: how much memory the process was allowed to use, and how to raise that limit. On Android the limit comes from `android:largeHeap="true"` **in the manifest of the app under test**, and the guidance says so, because declaring it in `src/androidTest/AndroidManifest.xml` — where it ends up in the manifest of the test apk — has no effect at all: an instrumentation process is created from the `ApplicationInfo` of the app under test. That's what the reporter of #2783 hit, with the analysis running out of memory at the 192 MB an app process gets without the flag. When the flag is already set, the guidance says so instead of suggesting it, and when the heap dumps were deleted, which the default `HeapDumpStorageStrategy.DeleteOnHeapDumpClose` does as soon as each traversal is over, it says how to keep them and detect heap growth from a computer instead.
    * 🔀 An out of memory failure of `findRepeatedlyGrowingObjects()` is now thrown as a `RuntimeException` carrying that guidance, with the original failure as its cause. It used to be thrown as is, which in practice already was a `RuntimeException` from one of Shark's hash maps rather than the `OutOfMemoryError` itself.
* 🔨 The guidance for a heap analysis that ran out of memory now says how much memory the analysis was allowed to use, and suggests `android:largeHeap="true"` only when it isn't set already.
* ✨ Heap growth detection is now documented: [Detecting heap growth](heap-growth.md). It covers what growth detection finds that `LeakAssertions.assertNoLeaks()` can't, how to read the report, how to tune the loop, where the heap dumps go and how to keep and pull them, why `android:largeHeap="true"` has to be declared in the manifest of the app under test, and how to run the detection from your computer with `shark-cli heap-growth` when a test process doesn't have the memory for it. Until now the only documentation was the release notes of the alpha it shipped in.
* 🐛 [#2792](https://github.com/square/leakcanary/issues/2792) `leakcanary-android-process` now declares the `com.squareup.leakcanary.process` namespace instead of `com.squareup.leakcanary`, which `leakcanary-android` also declares. An app depending on both — which is how running the analysis in a separate process is set up — failed to build with AGP 9 with *"Namespace 'com.squareup.leakcanary' is used in multiple modules and/or libraries"*. The two artifacts have shared that namespace since the process one was added, back when it replaced `leakcanary-android` rather than sitting alongside it, and the collision has been latent since 2.8 made it an add-on. AGP has reported duplicate namespaces since 8.2 and turned the report into an error in 9.0. Nothing else changes in the published AAR: it has no resources, no `R` class and no `BuildConfig`, and it names the service it declares by its fully qualified class name.
...
* 🐛 [#2869](https://github.com/square/leakcanary/pull/2869) The javadoc jars of the plain JVM artifacts — `shark`, `shark-android`, `shark-graph`, `shark-hprof`, `shark-log`, `leakcanary-core`, `leakcanary-gc`, `leakcanary-jvm-test`, `leakcanary-test-core` and `leakcanary-object-watcher` — shipped empty in 3.0 Alpha 9 and carry API documentation again. They're now built from Dokka's javadoc format, which is what the Android artifacts have always shipped, so every published artifact documents itself the same way.
...
* 🔨 A leaking object that another leaking object also retains now says so, with an `Also retained by leaking object <object id> (ClassName), which has its own leak trace` label. The path it's reached through isn't shown in any leak trace, because that path disappears when that other leak is fixed while the reported path survives fixing every other leak, so two leaks where one holds the other used to read as unrelated, or as duplicates of each other with oddly different paths. Only reported when retained heap sizes are computed, which is the default. Retained heap sizes are unaffected: the object stays reachable when that other leak is fixed, so it was never counted towards it.
    * 🔨 Labels that name several leaking objects of the same class are now grouped into one label per class, including the existing `Also retains leaking object …` labels, so a leaking object related to a dozen others doesn't bury the rest of its leak trace.
    * ⚠️ `PathFindingResults` takes an extra `alsoRetainingLeakingObjectsByLeakedObject` constructor argument, which is what carries that relationship out of a `ShortestPathFinder`.
* 🔨 [#2715](https://github.com/square/leakcanary/issues/2715) Retained heap sizes are now computed as part of the traversal that finds the paths from GC roots to the leaking objects, instead of building an approximate dominator tree for the whole heap and then walking it. Peak memory used by the analysis of our benchmark heap dump goes down 16%, from 6.52 MB to 5.47 MB.
    * 🔀 The dominator tree was an approximation that over attributed, sometimes by a lot (on one of our test heap dumps, 11 leaking objects were each reported as retaining the same 164 KB, 4 times more than the 42 KB the 31 leaking objects retain together), so the retained sizes LeakCanary reports change and are usually smaller. An object reachable from several leaking objects is now counted once, towards one of them, so the retained sizes of all the leaking objects add up to the size of the subgraph they retain together. The trade off is that fixing one of those leaks on its own frees less than the size reported for it, since the other leaking objects that also reach that object would keep it alive.
...
* 🐛 `ObjectDominators.buildDominatorTree()` and `ObjectDominators.renderDominatorTree()` now compute the exact dominator tree with Lengauer-Tarjan instead of the approximation they used to share with the leak analysis, so the retained sizes they report are correct. On one of our test heap dumps, `class android.view.DisplayListCanvas` was reported as retaining 380748 bytes in 2600 objects when it actually retains 221 bytes in 4 objects.
    * ⚠️ The `DominatorTree` class is gone, replaced by the new `shark.HeapDominatorTree`, which needs the whole graph up front.
    * ⚠️ `ObjectDominators.DominatorNode.retainedSize` is a `Long` instead of an `Int`. It sums the shallow sizes of a whole subtree, so the root of the tree holds the size of the reachable heap and silently wrapped negative past 2 GB. That was reachable in alpha 9 too, but the exact tree is built for tools running on a workstation, where heap dumps that large are ordinary.
* 🐛 A leak trace going through an entry of an object array reported the wrong index. The index was the position of that entry among the entries the analysis surfaces, which skip null entries and entries pointing to an object that isn't in the heap dump, rather than its position in the array, so each skipped entry ahead of it shifted the reported index down by one. The table of a hash based collection is mostly null entries, so a leak trace that goes through one — which is what LeakCanary reports when it doesn't recognize the collection and walks its internals instead of expanding it — said `[0]` whichever bucket the entry was really in. Leak fingerprints don't change: they already normalize every array index to `[x]`.
* 🐛 `ObjectDominators.buildDominatorTree()` and `ObjectDominators.renderDominatorTree()` only applied their `ignoredRefs` parameter when picking GC roots, and not when walking the heap. So they followed `WeakReference.referent`, `SoftReference.referent`, `PhantomReference.referent` and the finalizer list links no matter what was passed in, and reported memory as retained that garbage collection would reclaim. `ignoredRefs` now applies to the traversal too.
* 🐛 Heap growth detection computed the retained size of the objects it reports as growing from an approximate dominator tree built as the heap was traversed, an approximation that over attributes, sometimes by a lot. Retained sizes are now computed with the LeakShare metric from [BLeak](https://doi.org/10.1145/3192366.3192376), the paper heap growth detection is based on: the objects that are only reachable through the reported growing objects are credited to the growing objects that reach them.
    * 🔀 An object that stays reachable without going through a growing object isn't counted anymore, and an object held by several growing objects is now split evenly between them instead of being counted in full for each, so the retained sizes reported change and are usually smaller. They add up to the size of the subgraph the growing objects hold together. The trade off is that the size reported for a single growing object isn't a lower bound of what fixing that one growing object would free, since freeing shared objects requires fixing every growing object that holds them.
    ...
    * 🔨 That walk doubles the reads a traversal performs, so it now only runs for the last two heap dumps, which is all that reporting a retained size and its increase needs. On our test heap dump of a growing heap, the traversals before those two go back to 20006 reads instead of 40016. Callers that drive the traversals themselves get this by telling `InitialState` how many heap dumps they'll traverse.
    * ⚠️ `InitialState` takes a new optional `heapDumpCount` parameter, exposed by `HeapTraversalInput`, which changes the signature of the `InitialState(Int)` constructor. Kotlin callers that pass `scenarioLoopsPerGraph` still compile, but they have to be recompiled.
    * ✨ `ShortestPathObjectNode.exclusiveRetained` is the part of `retained` made of objects that no other growing object reaches, i.e. a lower bound of what fixing this one growing object would free, which `retained` isn't. Growing objects that hold the same data show up as a large `retained` with a small `exclusiveRetained`.
...
* ✨ A `HeapGraph` can now be read from several threads at the same time, so tools built on Shark can fan out the queries they run over a heap dump. Concurrent reads used to share a single record parsing buffer and an unsynchronized object record cache, and would fail with *"Buffer not fully consumed"* or read the wrong bytes. The `Sequence` instances a graph hands out are still meant to be read by a single thread: each thread should get its own.
    ...
    * 🔀 In a `GraphContext`, a null value and a missing key are now the same thing: setting a key to null removes it, so `contains()` then returns false for that key.
    * 🔨 `ConstantMemoryMetricsDualSourceProvider` updates each of its metrics atomically, so they still add up when the heap dump is read from several threads.
* ⚠️ `ConstantMemoryMetricsDualSourceProvider.randomAccessByteTravel` and `byteTravelRange` are gone, and the `travel=` and `range=` fields they filled in the `"Stats"` heap analysis metadata with them. Byte travel added up the distance between one read and the next, as a proxy for reads that need to pull a new part of the heap dump off storage. That proxy was a rotational disk model — on flash, distance costs nothing, and what does cost is loading a page that isn't in the page cache. It also over reported by orders of magnitude: on our benchmark heap dumps travel came out at 146x to 1375x the length of the file the analysis was reading, while the analysis in fact reads from fewer than a quarter of the file's pages, and 43% to 73% of its reads land in the page the previous read already loaded. `byteTravelRange` measured the distance between the lowest and highest position read, which was 77% to 78% of the file length on all three, so it said little that the file length didn't. `HprofIOPerfTest` now tracks page loading instead, freezing how many distinct 4 KiB pages an analysis reads from and how many times it reads a page, which is what a change to the analysis moves when it makes the heap dump more expensive to read.
* ✨ Work that reads a heap dump can now be canceled, by handing a `shark.CancelSignal` to `HprofHeapGraph.openHeapGraph()` or `HprofIndex.indexRecordsOf()`. Shark asks the signal on every read of the heap dump, including the reads served from its object record cache, and at the points in an analysis that run for a while without reading anything — indexing's four sorts, and the loops of the exact dominator tree that walk arrays rather than the heap dump. It then throws `shark.CanceledException`, so code built on a graph doesn't have to check anything itself to be interruptible. Cancellation stays cooperative on purpose: heap dump reads go through a `FileChannel`, which closes itself when a thread blocked on it is interrupted, so interrupting one of several threads reading a heap dump would leave that heap dump unreadable for all of them.
...
* 🔨 [#2774](https://github.com/square/leakcanary/issues/2774) `ServiceWatcher` no longer replaces the app's `IActivityManager` with a `java.lang.reflect.Proxy`. It needs to know when a service has finished `Service.onDestroy()`, and it used to learn that by watching for the `serviceDoneExecuting` call `ActivityThread` makes right after — which routed *every* `IActivityManager` call the process made, hundreds of unrelated framework calls, through LeakCanary's `InvocationHandler`, and put three LeakCanary frames in the stack trace of each. Framework and app bugs reachable through `IActivityManager` were therefore reported against LeakCanary: a `SecurityException` from `closeSystemDialogs` ([#2774](https://github.com/square/leakcanary/issues/2774)), a `TransactionTooLargeException` from `activityStopped` ([#2476](https://github.com/square/leakcanary/issues/2476)), an ANR ([#2618](https://github.com/square/leakcanary/issues/2618)) and a Play pre-launch non-SDK API warning ([#2199](https://github.com/square/leakcanary/issues/2199)). `ServiceWatcher` now posts a message from the `Handler.Callback` it already installs on `ActivityThread.mH` to catch `STOP_SERVICE`: `ActivityThread.handleStopService()` calls `Service.onDestroy()` while handling that message and everything after it synchronously, so a message posted from the callback runs once the service is destroyed. One hidden API hook instead of two, and LeakCanary is out of `IActivityManager` stack traces — which matters most to apps that ship `leakcanary-object-watcher-android` in release builds, as [documented](recipes.md#counting-retained-instances-in-release-builds).
    * 🔀 A destroyed service is now watched from the main thread message that follows the one destroying it, rather than from within that message, so watching starts one message later than it used to.
    * 🐛 `ServiceWatcher` stopped forwarding messages to a `Handler.Callback` already installed on `ActivityThread.mH` by something else, for every message whose `Message.obj` isn't an `IBinder` — which is most of them. That was collateral damage from the fix for [#2114](https://github.com/square/leakcanary/issues/2114), which returned early on those messages to avoid a `ClassCastException` on some Motorola ROMs.
* 🔀 `shark-cli dump-process` now asks the device to collect its garbage before writing the heap dump, with `am dumpheap -g`. Nothing else on that path collects: `am dumpheap` on its own writes whatever happens to be in the heap, and ART's hprof dumper suspends every thread rather than collecting, so a pulled dump held the live heap plus everything that had become garbage since the last collection. Measured on an API 29 emulator, dumping a real app's main process: 12.02 MB of the 92.9 MB of objects in the dump was unreachable, over 208733 objects, against 0.29 MB over 10467 objects for the same process dumped with `-g` a minute later. `-g` runs `System.gc()`, `System.runFinalization()` and `System.gc()` in the dumped process, the same trio LeakCanary runs before dumping its own heap, and it arrived in Android 8.1 — a device older than that is dumped without it, since it would refuse the whole command over an unknown option.
...
* 🔀 A binder stub — any `android.os.Binder` subclass — is now reported as **not leaking** in a leak trace, so a leak trace that runs through one is cut at the stub instead of somewhere above it. A stub stays in memory until the process on the other side gets GCed, so it outliving the activity or service it was made for is by design and there is nothing about it being there to fix. This says nothing about what the stub holds: a not-leaking status only spreads to what *holds* an object, never to what it holds, so every object below the stub keeps the status it had and one of them is usually the leak.
* 🔨 The leak trace label on a binder stub now sends you to what the stub *holds* instead of ending the investigation at the stub. It used to offer two ways out — "make it static", or, if the stub is a framework class, file a ticket — which leaves nothing to do in the common case where the stub is `android.os.ResultReceiver$MyResultReceiver` or another framework class and the object at fault is the one below it. A stub staying in memory until the process on the other side gets GCed is by design, so the stub is usually not the thing to fix; what is, is that a stub has to be a static class and every reference it was given has to be clearable and cleared when the work is done. So the label now says to read what the stub holds and decide object by object which of those should still be here, and gives the three things to do about it in order: make the stub static and clear what it holds at the end of its work if the stub is yours, since a compiler-generated `this$0` is what a non-static one looks like; file a ticket if it is framework or library code, because a stub you can't edit holding your object past its life is still a bug; and fix it from below anyway, since the reference that has to go is often further down the chain and in your code — if the stub holds your callback and your callback holds an activity, clearing that activity field in `onDestroy` ends the leak whoever owns the stub. Swapping a reference for a `WeakReference` is still not the fix, since that only makes the behavior depend on when a GC runs.
...
<manifest
  ...
  <!-- Performing the heap growth analysis in process requires more heap. This has to be the
       manifest of the app under test: an instrumentation process is created from the
       ApplicationInfo of the app under test, so largeHeap in src/androidTest/AndroidManifest.xml,
       which ends up in the manifest of the test apk, has no effect. -->
  <application
...
* **Large API rewrite** to improve usability. If you used the alpha with a customized configuration, there are breaking changes. Of note: LeakSentry became [AppWatcher](/leakcanary/api/object-watcher-android-core/object-watcher-android-core/leakcanary/-app-watcher/), RefWatcher became `ObjectWatcher`, AndroidExcludedRefs became [AndroidReferenceMatchers](/leakcanary/api/shark-android/shark-android/shark/-android-reference-matchers/), AnalysisResultListener became `OnHeapAnalyzedListener`, AndroidLeakTraceInspectors became [AndroidObjectInspectors](/leakcanary/api/shark-android/shark-android/shark/-android-object-inspectors/).
