# Releasing Shark Dive

Shark Dive is a desktop app, not a library, and it is released **separately from LeakCanary**. This
page is that process. [Releasing LeakCanary](releasing.md) is the other one, and the two share nothing but
the repository.

| | LeakCanary | Shark Dive |
| --- | --- | --- |
| Tags | `v3.0-alpha-10` | `shark-dive-1.0.0` |
...
`SHARK_DIVE_VERSION` is three integers and nothing else. Every installer format validates it, and
...
What follows is that **the release says it instead**. `release-shark-dive.yml` marks every release as a
prerelease and titles it `Shark Dive <version> (alpha)`. Drop that when the app is no longer alpha.

## Cutting a release

Set the version, tag it, and let CI do the rest. The workflow refuses to run if the tag and
`SHARK_DIVE_VERSION` disagree, so this order matters.

```bash
printf "Version being released (e.g. 1.0.1): " && read NEW_VERSION
git checkout main && git pull && \
git checkout -b shark_dive_$NEW_VERSION && \
...
git push origin shark-dive-$NEW_VERSION && \
gh run watch $(gh run list --workflow=release-shark-dive.yml --limit 1 --json databaseId --jq '.[].databaseId') --exit-status
```
...
asset of the rolling `shark-dive-latest` release — and only `promote-shark-dive.yml` writes it:

```bash
gh workflow run promote-shark-dive.yml -f version=$NEW_VERSION
```
...
not optional even though nothing about a Shark Dive window release touches the API reference: `docs/api` is
...
at `go/cpeticket` with the repository, the bundle ID (`com.squareup.leakcanary.shark-dive`), a release
...
Note that Munki compares versions using `CFBundleShortVersionString`, which is `SHARK_DIVE_VERSION` —
...
Fixed here, by `shark-dive-app/build.gradle.kts`, which deletes them from the app image. Worth
...
The service signed `Shark Dive.app` correctly — the Buildkite job passed and uploaded the signed zip —
and then the lambda failed working out where it had put it: `bad URI(is not URI?): "s3://…/Shark
Dive.app.zip"`, after a mac worker had done all the work. `destination_url` in
