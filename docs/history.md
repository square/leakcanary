...
LeakCanary was written by [Pierre-Yves Ricau](https://github.com/pyricau), who has maintained it ever
since 2015. A lot of what it is today came from other people though, and where a part of LeakCanary
exists because someone else built it or argued for it, this page tries to say so.
...
survives today was settled.

The first LeakCanary logo was a sprite lifted from Swipey Bird, a Flappy Bird clone built at a Square
hack week in 2014, in which you made the bird fly by swiping a credit card through a card reader.

<img src="../images/history-first-canary.gif" width="160" alt="The first LeakCanary logo: a pixel art bird sprite from Swipey Bird">

<img src="../images/history-swipey-bird.jpg" width="420" alt="The Swipey Bird poster from Square's Q3 2014 hack week">

A day later it was replaced by an icon of LeakCanary's own, assembled rather than drawn: a photograph
of a real canary, outlined in Photoshop, over a shield generated by Roman Nurik's free
[Android Asset Studio](https://romannurik.github.io/AndroidAssetStudio/icons-generic.html).

The outline was rough, so Romain Guy redrew the whole thing in a vector file — his version on the
right:

<img src="../images/history-icon-before-after.png" width="440" alt="The hand outlined icon next to Romain Guy's vector redraw of it">
...
It started from a list of complaints rather than a list of features, written down before any code was
touched. Some of that list was about the analysis, but the entries that changed how LeakCanary
*behaves* were about living with it:

!!! quote "LeakCanary 2"
    LeakCanary only processes one leak at a time, so if there are several leaks the phone will just
    alternate between dumping (freezing) and analyzing which is frustrating.

    Most people have no idea how much they can configure LeakCanary.

    LeakCanary is very customizable but that requires copying some library code and doing things the
...
[LeakCanary 2: Leaner, Better, Faster, Kotliner!](https://www.youtube.com/watch?v=LEX8dn4BLUw). The
week before it was the busiest week of the entire rewrite, at 67 commits, and two days before the tag
came a pull request titled *"Fixes based on using v2 with Android Makers"* — the app it was being
tested against was the conference's own.

There is a separate [upgrade guide](upgrading-to-leakcanary-2.0.md) for the API changes.

## Reading an obfuscated heap dump

A heap dump from a release build names classes `a.b.c`, which makes a leak trace useless. Being able
to read one arrived in three pieces over ten months of 2019 and 2020, all of them Michał Zieliński's:
Shark learned to parse a ProGuard mapping file and apply it while building a leak trace, a Gradle
plugin put that mapping file into the APK as an asset so that it would be on the device when it was
needed, and the same deobfuscation became an option on the `shark` command line for a heap dump you
already have.
