...
class EvalScenariosTest {
  ...
  fun `every scenario names its key once the unknown zone is closed`() {
    EvalScenarios.all(repositoryRoot).forEach { scenario ->
      ...
      HeapExplorer.open(file).use { explorer ->
        val chain = explorer.chainToTheFirstLeak()
        ...
        val solved = explorer.tree.rootPathTo(
  ...
  fun `no scenario names its key before anybody has read anything`() {
    EvalScenarios.all(repositoryRoot).forEach { scenario ->
      ...
      HeapExplorer.open(file).use { explorer ->
        ...
        assertThat(explorer.chainToTheFirstLeak().faultyReference())
  ...
  fun `every scenario is a leak the heap dump finds on its own`() {
    EvalScenarios.all(repositoryRoot).forEach { scenario ->
      ...
      HeapExplorer.open(file).use { explorer ->
        ...
        assertThat(explorer.tree.findLeaks().leakingObjectCount)
  ...
  private fun HeapExplorer.chainToTheFirstLeak(): RootPath {
