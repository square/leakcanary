...
/**
 * Asks whether a newer Shark Explorer has been released, and answers with [AvailableUpdate] or null.
 *
...
internal class UpdateCheck(
  private val currentVersion: String = SharkExplorerVersion.current,
  private val fetchManifest: () -> String? = ::fetchLatestManifest
) {

  fun check(): AvailableUpdate? {
    if (currentVersion == SharkExplorerVersion.UNKNOWN_VERSION) {
 ...
 * tags and the explorer on `shark-explorer-*` tags, and GitHub has one "latest" per repository, so that
 ...
 * a build and telling everyone about it stay two separate acts. See docs/releasing-shark-explorer.md.
 */
...
/**
 * The one asset of the rolling `shark-explorer-latest` release, rewritten by whichever release is being
...
private const val LATEST_MANIFEST_URL =
  "https://github.com/square/leakcanary/releases/download/shark-explorer-latest/latest.properties"
