...
class UpdateCheckTest {
  ...
  @Test fun `a run that does not know its version is never offered an update`() {
    val update = UpdateCheck(
      currentVersion = SharkExplorerVersion.UNKNOWN_VERSION,
  ...
  /**
   * The manifest as `.github/workflows/promote-shark-explorer.yml` actually writes it, comments and all.
   *
  ...
  @Test fun `the manifest the promote workflow writes is one this reads`() {
    val written = """
      ...
      version=1.2.0
      releaseUrl=https://github.com/square/leakcanary/releases/tag/shark-explorer-1.2.0
  ...
  private companion object {
    const val RELEASE_URL = "https://github.com/square/leakcanary/releases/tag/shark-explorer-0.2.0"
  }
}
