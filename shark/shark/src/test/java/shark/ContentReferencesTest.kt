...
/**
 * The dominator tree follows content references, so content shared by several objects is attributed
 * to a single dominator instead of being counted once per object holding it.
 *
 * See https://github.com/square/leakcanary/issues/2700, where sharing inflated the total retained
 * size past [Int.MAX_VALUE].
 */
class ContentReferencesTest {
  ...
  @Test fun `a real jvm heap dump does not retain more than it holds`() {
    ...
    check(sharing.size == SHARING_COUNT && bigString.length == 1_000_000)
