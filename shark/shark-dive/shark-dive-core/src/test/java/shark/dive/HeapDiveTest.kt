...
import shark.dive.ReachabilityStrength.CACHE
...
class HeapDiveTest {
  ...
  @Test fun `the root is the whole heap dump`() {
    testFolder.openTestHeapDump().use { dive ->
      ...
      assertThat(tree.weight(tree.root)).isEqualTo(dive.sizes.totalByteCount)
    }
  }
  ...
  @Test fun `an instance is labelled with its simple class name`() {
    testFolder.openTestHeapDump().use { dive ->
      val holder = dive.tree.findByLabel("Holder")
  ...
  @Test fun `a bitmap is a cell with its pixels on it`() {
    HeapDive.open(testFolder.pixelBitmapHeapDump()).use { dive ->
      val tree = dive.tree
  ...
  @Test fun `an object two others hold is dominated by the whole heap dump`() {
    HeapDive.open(testFolder.sharedPayloadHeapDump()).use { dive ->
      val tree = dive.tree
  ...
  @Test fun `a cache that evicts is not what keeps an image in memory`() {
    HeapDive.open(testFolder.coilCachedImageHeapDump(alsoShownByATile = true)).use { dive ->
      val tree = dive.tree
  ...
  @Test fun `an image nothing but a cache holds is reachable at the cache strength`() {
    HeapDive.open(testFolder.coilCachedImageHeapDump(alsoShownByATile = false)).use { dive ->
      ...
      assertThat(dive.sizes.byteCountByStrength.getValue(CACHE)).isGreaterThan(PAYLOAD_BYTE_SIZE)
  ...
  @Test fun `a value nothing but a cache with a map of them holds is reachable at the cache strength`() {
    HeapDive.open(testFolder.mapCachedPayloadsHeapDump(alsoHeldByATile = false)).use { dive ->
      // One payload per cache and not a byte more: the framework's LruCache, and Glide's as the subclass
      // an app really has.
      assertThat(dive.sizes.byteCountByStrength.getValue(CACHE)).isEqualTo(2 * PAYLOAD_BYTE_SIZE)
  ...
  @Test fun `a reference from a cache with a map of them is not a way of holding what it caches`() {
    HeapDive.open(testFolder.mapCachedPayloadsHeapDump(alsoHeldByATile = true)).use { dive ->
      val tree = dive.tree
  ...
  @Test fun `what only holds an object until the runtime is done with it is on no path`() {
    HeapDive.open(testFolder.lastResortHoldersHeapDump()).use { dive ->
      val tree = dive.tree
  ...
  @Test fun `a path that leads back to the object is not a way of holding it`() {
    HeapDive.open(testFolder.cyclicHolderHeapDump()).use { dive ->
      val tree = dive.tree
      val view = tree.findByLabel("View")
  ...
  @Test fun `the shortest way a gc root holds an object marks what dominates it`() {
    HeapDive.open(testFolder.twoWaysToOnePayloadHeapDump()).use { dive ->
      val tree = dive.tree
      val payload = tree.findByLabel("Object[]")
  ...
  @Test fun `opening a file that is not a heap dump fails`() {
    ...
    assertThatThrownBy { HeapDive.open(notAHeapDump) }
  ...
  @Test fun `opening a file that is not there says where it looked`() {
    ...
    assertThatThrownBy { HeapDive.open(missing) }
  ...
  @Test fun `the loaded classes gather under java lang Class`() {
    HeapDive.open(testFolder.crowdedRootHeapDump(withJavaLangClass = true)).use { dive ->
      val tree = dive.tree
      val children = tree.children(tree.root)
      val groups = children.mapNotNull { tree.groupOrNull(it) }
