...
/**
 * What Shark Dive makes of a dependency injection singleton, in a heap dump of a real JVM holding real
 ...
 * [JvmReferenceStrengthTest] gives. To open Shark Dive on the same heap dump these assertions ran
 * against:
 *
 * ```
...
class DependencyInjectionOwnerTest {
  ...
  @Test fun `a Dagger singleton is held by the provider its component caches it in`() {
    val singleton = dive.tree.onlyInstanceOf("DaggerSingleton")
    ...
    assertThat(dive.tree.dominatorOf(singleton.objectId)!!.label).isEqualTo("DoubleCheck")
    assertThat(dive.tree.rootPathTo(singleton.objectId).stepLabels().takeLast(2)).containsExactly(
  ...
  @Test fun `a Metro singleton is held by the provider its graph caches it in`() {
    val singleton = dive.tree.onlyInstanceOf("MetroSingleton")
  ...
  @Test fun `a component collects the bytes of every singleton it caches`() {
    ...
    listOf(
      ...
      val component = dive.tree.onlyInstanceOf(componentName)
      val singleton = dive.tree.onlyInstanceOf(singletonName)
      ...
      assertThat(dive.tree.dominatorLabelsOf(singleton.objectId))
        .startsWith("DoubleCheck", componentName)
      assertThat(dive.tree.weight(component.objectId))
        .isGreaterThan(dive.tree.weight(singleton.objectId))
    }
  }

  @Test fun `a singleton whose provider something else holds too is held by neither`() {
    val shared = dive.tree.onlyInstanceOf("SharedProviderSingleton")
    ...
    assertThat(dive.tree.dominatorOf(shared.objectId)!!.label).isEqualTo("DoubleCheck")
    assertThat(dive.tree.dominatorLabelsOf(shared.objectId)).doesNotContain("DaggerComponent")
  }
  ...
  @Test fun `a provider nothing has asked yet holds no singleton of its own`() {
    ...
    val sentinelIds = dive.tree.instancesOf("DoubleCheck")
      .mapNotNull { doubleCheck ->
        dive.tree.summarize(doubleCheck.objectId)
          .fields
          .single { it.name == "instance" || it.name == "_value" }
          .inspectableObjectId
      }
      .filter { dive.tree.label(it) == "Object" }
    ...
    sentinelIds.forEach { sentinelId ->
      ...
      assertThat(dive.tree.dominatorOf(sentinelId)!!.label).isNotEqualTo("DoubleCheck")
      assertThat(dive.tree.independentPathsFromRoots(sentinelId).paths.map { it.stepLabels().last() })
        .contains("UNINITIALIZED → Object")
    }
  }

  @Test fun `a singleton whose component is gone is held by what points at it after all`() {
    // The dump is written without collecting, so the dropped component is still in it, and Shark Dive
    ...
    assertThat(dive.tree.onlyInstanceOf("DroppedComponent").strength).isEqualTo(UNREACHABLE)
    val dropped = dive.tree.onlyInstanceOf("DroppedSingleton")
    ...
    assertThat(dive.tree.dominatorOf(dropped.objectId)!!.label).isEqualTo("DroppedInjectionSite")
    assertThat(dive.tree.rootPathTo(dropped.objectId).stepLabels().takeLast(2)).containsExactly(
