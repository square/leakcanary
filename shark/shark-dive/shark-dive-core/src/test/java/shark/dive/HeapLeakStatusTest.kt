...
class HeapLeakStatusTest {
  ...
  @Test fun `a status set by hand is what the panel says instead`() {
    ...
    HeapDive.open(dump.file).use { dive ->
      val summary = dive.tree.summarize(
  ...
  @Test fun `an object holds the ones it reaches and not the other way round`() {
    ...
    HeapDive.open(dump.file).use { dive ->
      assertThat(dive.tree.reaches(dump.activityObjectId, dump.windowObjectId)).isTrue()
      assertThat(dive.tree.reaches(dump.windowObjectId, dump.activityObjectId)).isFalse()
    }
  }
  ...
  @Test fun `an object does not hold itself`() {
    ...
    HeapDive.open(dump.file).use { dive ->
      assertThat(dive.tree.reaches(dump.activityObjectId, dump.activityObjectId)).isFalse()
    }
  }
  ...
  @Test fun `an address that is no object of this heap dump reaches nothing`() {
    ...
    HeapDive.open(dump.file).use { dive ->
      assertThat(dive.tree.reaches(NOT_AN_ADDRESS, dump.windowObjectId)).isFalse()
      assertThat(dive.tree.reaches(dump.activityObjectId, NOT_AN_ADDRESS)).isFalse()
    }
  }
  ...
  @Test fun `objects that hold each other each reach the other`() {
    ...
    HeapDive.open(dump.file).use { dive ->
      assertThat(dive.tree.reaches(dump.wrapperObjectId, dump.taskObjectId)).isTrue()
      assertThat(dive.tree.reaches(dump.taskObjectId, dump.wrapperObjectId)).isTrue()
    }
  }
  ...
  @Test fun `an object set to leaking is a leak, and what it holds is folded under it`() {
    ...
    HeapDive.open(dump.file).use { dive ->
      val holderObjectId = dive.tree.findByLabel(HOLDER_CLASS_NAME.substringAfterLast('.')).objectId
      ...
      assertThat(dive.tree.findLeaks().leakingObjectIds).containsExactly(dump.activityObjectId)
  ...
  @Test fun `the leaks are what the heap dump says again once a status is taken off`() {
    ...
    HeapDive.open(dump.file).use { dive ->
      dive.tree.findLeaks(overrides(dump.activityObjectId, EXPECTED, "kept on purpose"))
