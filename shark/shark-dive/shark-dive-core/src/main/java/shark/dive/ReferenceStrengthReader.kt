...
import shark.dive.ReachabilityStrength.THREAD_LOCAL
 ...
 * objects only ever appear as flattened children of the structure holding them. A Shark Dive window needs
...
internal class ReferenceStrengthReader(private val graph: HeapGraph) {
   ...
   * metadata reading as uncollected garbage, which was 88% of everything Shark Dive called garbage.
   */
  ...
  companion object {
    ...
    /**
     * The fields a cache holds its entries in, which Shark Dive treats as [CACHE] rather than as
     ...
     * than a Shark Dive window's. Besides the referents, it drops the `prev`, `next` and `element` links of the
