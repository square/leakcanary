 ...
 * rather than collecting. Without it a Shark Dive window opens on a heap that is mostly garbage nobody can act
...
class DeviceHeapDumps(
   ...
   * debugger to offer gets. `JdwpBitmaps` in `shark-dive-jdwp` is the one there is.
   */
   ...
   * is a dump with garbage in it rather than no dump. `JdwpGc` in `shark-dive-jdwp` is the one there
   * is.
   */
   ...
   *
   * The caller owns the file and it is deliberately not deleted: Shark Dive reads a heap dump lazily
  ...
  fun fetchBitmaps(
    ...
    val localFile = File.createTempFile("shark-dive-bitmaps", ".hprof")
    try {
      pullHeapDump(
        device = device,
        process = process,
  ...
  private fun pullHeapDump(
    ...
    // next one reads.
    val remotePath = "$REMOTE_DIRECTORY/shark-dive-${process.processId}-${System.nanoTime()}.hprof"
 ...
 *
 * An interface here and implemented in `shark-dive-jdwp` because the one implementation is a JDI
 ...
 * `shark-dive-jdwp` for the same reason as that one: `com.sun.jdi` is part of a desktop JDK rather
