 ...
 *
 * Shark's own strings rather than the matcher it came from, so that what Shark Dive hands its UI stays
 * Shark Dive's own model — and so that two of them can be compared to gather the objects one known leak
 * is holding.
 */
