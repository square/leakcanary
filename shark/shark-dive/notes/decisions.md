...
Why Shark Dive is built the way it is. Append as decisions are made; correct entries that stop
...
`shark-dive-app` opts out of the root build script's Java 8 config and targets Java 17.
`shark-dive-core` stays on Java 8 so Android can consume it.

## Two modules, not one

`shark-dive-core` holds the whole pipeline — heap dump → dominator tree → treemap layout → hit
testing → navigation state — as plain JVM code with no Compose dependency. `shark-dive-app` is
...
`shark-dive-jdwp` holds one class, `JdwpBitmaps`, and exists because of what it imports:
...
bitmap's pixels ([bitmaps.md](bitmaps.md)); this module knows *how*. `shark-dive-app` is where the two
are put together, which is what a wiring layer is for. A module rather than dropping the class into
`shark-dive-app` because "this needs a JDK" is a dependency boundary, and a boundary that only exists
...
`HeapDumpSession` owns a single thread executor and a `HeapDive`, and `read { }` is the only way
...
analysis — so Shark Dive doesn't have to find cancellation points of its own. `HeapDumpSession` opens
...
what it did to `~/.shark-dive/logs`, and the reads are logged with their durations: without them a
report is a guess about which of half a dozen heap dump reads was slow, failed, or never came back.

`SessionLog` lives in `shark-dive-core` and not in `shark-log`, next to `SharkLog` itself, because
...
`Shark Dive closed`, which is how a session that ended is told from one that was killed.
...
What follows: `heapDumpFile` is a window's, not `DiveApp`'s, because the window title is the file
name — several windows all called after the app say nothing about which one to switch to, and the OS
window list is the only way between them. `diveWindows` and `openHeapDump` are plain state so the
...
`HeapDumpDive` that the four of them go through.
...
above it testable without a device — `FakeAdb` in `shark-dive-core`'s tests answers by command
...
both. The difference is what happens to the file: a dump taken to be read is kept, because Shark Dive
reads it lazily for as long as it's open and a dump that took a minute is worth reopening, while
...
collection, and Shark Dive draws all of it under `Unreachable`.
...
difference in what Shark Dive can *see*: the same live heap, with 18 MB of uncollected objects beside it
...
`ActivityThread$ActivityClientRecord` that holds it, so Shark Dive can't walk LeakCanary's ten-step
...
tab: two tabs on one place are one note, and both show what the other is typing, since `DiveNotes` hands out
...
ever showing the loss. `DiveNotes` is one per run, hands out one `HeapDumpNotes` per dump and one
...
**Markdown files in `~/.shark-dive/notes`, a directory per dump.** Not beside the heap dump: dumps are
...
- **Four outcomes, and the first is nearly always the one.** A window of this run has that dump: that window.
  ...
  unknown here: ask for the file. `DiveWindows.open`.
...
screen reader gets for a canvas — and `DiveAppTest.viewBounds` reads its bounds, so every click is
