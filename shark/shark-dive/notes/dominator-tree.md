...
## How Shark Dive uses it

`HeapDive.open` calls `HeapDominatorTree.buildFor` with a `WeakeningAwareReferenceReader` wrapping
...
- **Deliberately not `JdkReferenceMatchers.REFERENCES`**, which is that list plus the `prev`, `next` and
  ...
  `FinalizerReference`s and 3392 of its 3553 `Cleaner`s, a fifth of everything Shark Dive called
...
- `AndroidReferenceMatchers` mostly suppresses noise so a leak trace stays readable. Those are real
  ...
  `HashMap` actually holds has to be a node of its own or its bytes land nowhere. Shark Dive *adds*
...
worth seeing, which is why the tree holds those objects rather than Shark Dive only following what the
...
The mechanics fall out of the weak reference machinery that was already there, because the rule
Shark Dive wants is exactly the rule for a weak reference: **the target's strength decides**. A weakening
...
mid-tree child would become unreachable, Shark Dive would call live objects garbage, and the same rule
...
### The virtual references Shark Dive adds
...
- **All or nothing per table.** An array only partly handed out from its groups would leave the rest
  reachable through nothing, which Shark Dive would draw as uncollected garbage. So a table that doesn't
...
the runtime's internals, none of which can explain a leak, which is what those defaults are for. Shark Dive
has to say where every object is held, so `HeapDive` opens with `HprofRecordTag.rootTags`
...
That's affordable on a phone but comfortable on desktop, which is part of why Shark Dive is a
