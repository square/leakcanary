...
Implemented in `shark-dive-core`: `Squarify.kt` (row layout), `TreemapLayout.kt` (adaptive depth
...
`HeapDiveDumps` is built up there for exactly this — `dump { }` takes a `firstObjectId` so that a test
...
Keeping all of that pure functions in `shark-dive-core` is what makes it testable — see
