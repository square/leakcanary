 ...
 *
 * The markdown itself is `NoteTest` in `shark-dive-core`, and where a note is kept is `NoteFileTest`,
...
class NoteSectionTest {
  ...
  @Test fun `a note is never dragged so tall that its own edge leaves the window`() {
    ...
    diveUiTest(height = SHORT_WINDOW) {
      openHeapDump()
      startNote()
  ...
  @Test fun `one place is one notepad however the heap dump path is spelled`() {
    val notes = DiveNotes(testFolder.newFolder("notes"))
    ...
    setContent {
      MaterialTheme {
        DiveApp(
          ...
          // A directory of this test's, never `~/.shark-dive`: a test that saved into the real one
