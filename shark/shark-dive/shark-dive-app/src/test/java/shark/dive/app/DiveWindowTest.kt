 ...
 * is a file and a title until [DiveApp] opens it.
 */
class DiveWindowTest {
  ...
  @Test fun `a window is named after the heap dump it shows`() {
    ...
    assertThat(DiveWindow(null).title).isEqualTo(APP_NAME)
  }

  @Test fun `a run given a title says it in front of every window name`() {
    val windows = diveWindows(opening(FIRST_DUMP, titlePrefix = TITLE))

    // Which is what tells one Shark Dive window from another, where a heap dump opened twice for two reasons is
    // otherwise the same window twice.
    assertThat(windows.single().title).isEqualTo("$TITLE · ${FIRST_DUMP.name}")
    assertThat(diveWindows(noHeapDumps(titlePrefix = TITLE)).single().title)
  ...
  @Test fun `a heap dump opened into a window of its own keeps the run's title`() {
    ...
    // Every window of one Shark Dive window belongs to whatever that Shark Dive window was started for, however many heap
  ...
  @Test fun `every window answers to an id of its own`() {
    val windows = diveWindows(opening(FIRST_DUMP, FIRST_DUMP))
  ...
  @Test fun `a link about a name two open windows share asks which of them`() {
    ...
    val windows = diveWindows(opening(pixel, emulator))
  ...
  private fun agentHeapDumps(windows: DiveWindows) = WindowAgentHeapDumps(
    windows = windows,
  ...
  private fun noHeapDumps(titlePrefix: String? = null) =
    DiveArguments(heapDumpFiles = emptyList(), titlePrefix = titlePrefix)

  private fun opening(
    vararg heapDumpFiles: File,
    titlePrefix: String? = null
  ) = DiveArguments(heapDumpFiles = heapDumpFiles.toList(), titlePrefix = titlePrefix)
   ...
   */
  private fun diveWindows(arguments: DiveArguments) =
    diveWindows(arguments, heapDumpPaths)
