 ...
 * dives open at once is the normal way this app is used — one per piece of work, often on the same heap
...
internal object DeepLinkPeers {
   ...
   */
  fun listen(windows: DiveWindows): Closeable {
   ...
   * [DiveWindows.open] — so a link with nobody to take it lands in the run the OS chose, which is the
   * installed app.
   *
  ...
  fun follow(
    ...
    Thread({
      ...
      // is, since a link says the dump's name and not where it is. See [DiveWindows.open].
   ...
   * belongs to whoever ends up with the link rather than to whoever passed it on. See [DiveWindows.open].
   */
  ...
  private fun write(
    ...
    file.outputStream().use { properties.store(it, "Where this Shark Dive run answers links") }
  ...
  private fun peerDirectory(): File = File(SHARK_DIVE_DIRECTORY, "runs")
  ...
  private const val THREAD_NAME = "shark-dive-links"
}
