...
/**
 * The app's own version, from `SHARK_DIVE_VERSION` rather than the repo wide `VERSION_NAME`: 
 * Shark Dive is released on its own tags, and jpackage would reject `VERSION_NAME` anyway. See
 * `gradle.properties`.
 */
val diveVersion = property("SHARK_DIVE_VERSION").toString()
...
dependencies {
  implementation(projects.shark.sharkDive.sharkDiveCore)
  // Reads the bitmaps of a live process off the Android versions whose heap dumps can't carry them.
  implementation(projects.shark.sharkDive.sharkDiveJdwp)
  // What an agent asks this app's windows through, and the pipe that reaches them. See DiveAgents.kt.
  implementation(projects.shark.sharkDive.sharkDiveAgent)
...
compose.desktop {
  application {
    ...
    nativeDistributions {
      ...
      // docs/releasing-shark-dive.md.
      packageName = "Shark Dive"
      ...
      macOS {
        ...
        bundleID = "com.squareup.leakcanary.shark-dive"
        ...
        // arrives there. Try links against `createDistributable`, per shark/shark-dive/AGENTS.md.
        infoPlist {
          extraKeysRawXml = """
            ...
            <array>
              <dict>
                ...
                <string>com.squareup.leakcanary.shark-dive.link</string>
      ...
      // what `shark-dive-jdwp` attaches to a live app with.
 ...
 * local check found it. See docs/releasing-shark-dive.md.
 *
 ...
 * look.
 */
abstract class RunNamedDive : DefaultTask() {
  ...
  /** Shark Dive's own command line, spelled the way `run` takes it. */
  @get:Input
  @get:Optional
  @get:Option(option = "args", description = "Shark Dive's command line, as `run` takes it.")
  ...
  fun launch() {
    ...
    logger.lifecycle("Launched \"$name\". It logs to ~/.shark-dive/logs, and to $output if it can't.")
  }
  ...
  private companion object {
    const val EXECUTABLE_NAME = "shark-dive"
    const val ICON_FILE_NAME = "shark-dive-icon.icns"

    /** Not the packaged app's, which names a real installed app rather than a bundle in a build folder. */
    const val BUNDLE_IDENTIFIER = "shark.dive.app.named"
