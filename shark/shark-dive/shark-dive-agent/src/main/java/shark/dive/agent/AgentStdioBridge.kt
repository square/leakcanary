 ...
 * `.app` on the machine is both the window and the bridge to it. See `shark.dive.app.main`.
 *
...
object AgentStdioBridge {
  ...
  private fun pump(
    ...
    say("Talking to Shark Dive run ${run.pid}")
    ...
    val answers = Thread({
      ...
      try {
        ...
        say("Shark Dive run ${run.pid} closed the connection")
      } catch (throwable: Throwable) {
        if (!ending.get()) {
          say("Shark Dive run ${run.pid} stopped answering: $throwable")
        }
      }
      out.flush()
    }, "shark-dive-agent-answers").apply {
      isDaemon = true
      start()
    }
    ...
    while (true) {
      ...
      toApp.println(line)
      if (toApp.checkError()) {
        say("Shark Dive run ${run.pid} went away")
        return NOTHING_TO_TALK_TO
      }
    }
  ...
  /** What the command line says to pick a run by process id. See `shark.dive.app.DiveArguments`. */
...
internal fun waitForRun(
  ...
  while (true) {
    ...
    if (waited >= deadline) {
      say(
        if (pid == null && opened) {
          "A Shark Dive run was started and has not published itself in " +
            "${OPENING_WAIT_MILLIS / 1000} seconds, so something went wrong opening it. Its log is in " +
            "the newest file under ~/.shark-dive/logs."
        } else if (pid == null) {
          "No Shark Dive is running, so there is no heap dump to investigate. Open one — every run " +
